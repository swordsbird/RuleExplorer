...
class ModelUtil():
    ...
    def get_sum(self, key, left, right, dl = 0, dr = 0):
        key = int(key)
        try:
            left = bisect.bisect_left(self.ordered_vals[key], left - 1e-6) - 1 + dl
    ...
    def anomaly_score(self, paths):
        X_test, y_test = self.get_rule_matrix(paths)
        y_test = y_test.astype(int)
        score = self.anomaly.score(X_test, y_test)
        return score

    def init_anomaly_detection(self):
        X, y = self.get_rule_matrix()
        y = y.astype(int)
        idx = [i for i, p in enumerate(self.model.paths) if p['coverage'] > self.min_coverage]
        self.anomaly = LRAnomalyDetection(X[idx], y[idx], self.model.n_classes)
    ...
    def get_feature_representation(self, features, paths):
        feature_names = [feature['name'] for feature in features]
        feature_importances = [feature['importance'] for feature in features]
        ...
        average = np.zeros((n_features)).astype('float')
        data_table = self.data_table
        ...
        for i, feature in enumerate(self.features):
            if is_categorical[i]:
                if i in self.model.to_category_idx:
                    ...
                    average[i] = (data_table[feature] == 1).sum() / n_samples

        for feature in feature_val_idxs:
            idx = feature_val_idxs[feature]
            default_values[feature] = average[idx]
            default_type[feature] = 'category'
        
        for i in self.feature_pos:
            feature, idx = self.feature_pos[i]
            if feature not in feature_val_idxs:
                for j in range(len(feature_names)):
                    if feature_names[j] == feature:
                        feature_index[j] = i
                        default_type[feature] = features[j]['dtype']
                        if default_type[feature] == 'number':
                            default_values[feature] = [0, 1]
                        else:
                            vmin = data_table[feature].min()
                            vmax = data_table[feature].max()
                            default_values[feature] = []
                            for k in range(vmin, vmax + 1):
                                val = (data_table[feature] == k).sum() / n_samples
        ...
        for row_i, p in enumerate(paths):
            ...
            for i in m:
                ...
                if default_type[feature] == 'category':
                    current[i] = copy.deepcopy(default_values[feature])
                    ...
                    left = m[j][0]
                    right = m[j][1]
                    left = self.get_sum(j, 0, left - 1e-6, 1, 0) / len(self.model.X)
                    right = self.get_sum(j, 0, right, 1, 0) / len(self.model.X)
                    current[i] = [left, right]
            for i, feature in enumerate(feature_names):
                cond = np.array(current.get(i, default_values[feature])).astype(np.float)
                cond *= feature_importances[i]
                row.append(cond)
        ...
        defaults = []
        for i, feature in enumerate(feature_names):
            defaults.append(default_values[feature])

        return conds, features, defaults

    def get_rule_matrix(self, paths = None):
        feature_pos = copy.deepcopy(self.feature_pos)
        is_feature_categorical = {}
        feature_val_idxs = {}
        for i in self.feature_pos:
