...
class Forest():
    ...
    def find_the_min_set(self):
        ...
        target_correct_vector = np.zeros(shape=self.df.shape[0])
        ...
        while (go_on):
            ...
            target_matched_vector = np.logical_or(target_matched_vector, self.rule_matched_table[rid])
            target_correct_vector = np.logical_or(target_correct_vector, np.logical_and(mask, self.rule_matched_table[rid]))
            target_mask_vector = np.logical_or(target_mask_vector, self.rule_matched_table_all[rid])
            # print('all', np.sum(target_mask_vector), 'matched', np.sum(target_matched_vector), 'correct', np.sum(target_correct_vector))
            del D[rid]
            if (len(D) == 0):
                go_on = False
            else:
        ...
        # get coverage
        self.fidelity = float(target_correct_vector.sum()) / target_matched_vector.shape[0]
        ...
        return {'rules': target_rule_set, "tot_rule": len(self.rules), "correct_coverage": self.coverage, 'fidelity': self.fidelity,
